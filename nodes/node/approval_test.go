...
func TestApprovalAll_PrepareSuspendUsesPerApproverMultiSignal(t *testing.T) {
	...
	spec, err := sh.PrepareSuspend(context.Background(), &node.Input{
	...
	if spec.Mode != node.ModeMultiSignal {
		t.Fatalf("Mode = %v, want ModeMultiSignal", spec.Mode)
	}
...
func TestApprovalAll_PrepareSuspendKeepsLegacySharedSignalAfterPartialDecision(t *testing.T) {
	...
	if spec.Mode != node.ModeSignal {
		t.Fatalf("Mode = %v, want ModeSignal", spec.Mode)
	}
...
func TestApprovalAll_OnResumeConsumesMultiSignalPayloadInApproverOrder(t *testing.T) {
	sh := node.Approval([]string{"alice", "bob"}, node.ApprovalAll)
	out, err := sh.OnResume(context.Background(), &node.Input{
		NodeName: "SecurityApproval",
		Params: map[string]any{
			"approvers": []any{"alice", "bob"},
			"mode":      "all",
...
func approvalHandler(t *testing.T) node.SuspendingHandler {
	t.Helper()
	h, ok := node.Lookup(node.ApprovalNodeType)
	if !ok {
		t.Fatal("approval node is not registered")
	}
	sh, ok := h.(node.SuspendingHandler)
	if !ok {
...
func approvalInput(mode node.ApprovalMode, data map[string]any) *node.Input {
	return &node.Input{
		Params: map[string]any{
			"approvers": []any{"alice", "bob"},
			"mode":      string(mode),
...
func approvalSignal(approver string, action string, comment string) *node.SignalPayload {
	return &node.SignalPayload{
		Triggered: node.SignalReceived,
