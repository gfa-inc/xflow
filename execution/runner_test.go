...
func TestRunner_WithCredentialResolverAttachesToInput(t *testing.T) {
	...
	resolver := func(t namespace.Namespace, name string) map[string]any {
		if name == "db" {
			return want
		}
		return nil
...
// TestRunner_CredentialResolverReceivesNamespaceFromContext pins that the namespace
// carried by the execution context is forwarded to the credential resolver.
// This is the local-runner counterpart to the dispatcher injecting namespace into
// Assignment and the asynq consumer restoring it before Execute.
func TestRunner_CredentialResolverReceivesNamespaceFromContext(t *testing.T) {
	rec := newCredRecordingHandler()
	resolver := func(t namespace.Namespace, name string) map[string]any {
		if name != "db" {
			return nil
		}
		return map[string]any{"namespace": string(t)}
	}
	runner := NewRunner(singleHandlerRegistry{handler: rec}, WithCredentialResolver(resolver))

	ctx := namespace.WithNamespace(context.Background(), "namespace-acme")
	lease := &engine.TaskLease{
		Task:     engine.Task{ExecutionID: "exec-namespace", NodeName: "probe"},
		Input:    &types.Input{ExecutionID: "exec-namespace", NodeName: "probe"},
		NodeType: "test.cred-probe",
	}
	if _, err := runner.Execute(ctx, lease); err != nil {
		t.Fatalf("Execute() error = %v", err)
	}
	got, ok := rec.observed("exec-namespace")
	if !ok {
		t.Fatal("handler observed nil credential, want namespace-scoped resolver value")
	}
	if got["namespace"] != "namespace-acme" {
