...
if child.status == 'running' then
    ...
    redis.call('HSET', KEYS[3], ARGV[5], cjson.encode(child))
end
...
var _ engine.LeaseExpander = (*redisState)(nil)
var _ engine.DurableLeaseExpander = (*redisState)(nil)

func (s *redisState) BeginTaskExpansionWithOutbox(ctx context.Context, lease *engine.TaskLease, children []engine.SubExecution, entries []engine.OutboxEntry) (bool, error) {
...
func (s *redisState) BeginTaskExpansion(ctx context.Context, lease *engine.TaskLease) (bool, error) {
	if lease == nil {
		return false, engine.ErrInvalidLeaseToken
	}
	ttl := s.getExecTTL(lease.Task.ExecutionID)
...
func (s *redisState) CreateExpandedSubExecution(ctx context.Context, lease *engine.TaskLease, sub *engine.SubExecution) (bool, error) {
...
func (s *redisState) CompleteExpandedSubExecution(ctx context.Context, lease *engine.TaskLease, childExecID types.ExecutionID, status types.ExecutionStatus, result map[string]any) (bool, bool, []map[string]any, error) {
