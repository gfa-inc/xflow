...
var _ engine.AtomicStateStore = (*redisState)(nil)
var _ engine.LegacyNodeCommitter = (*redisState)(nil)
var _ engine.OutboxFailureRecorder = (*redisState)(nil)
var _ engine.OutboxMetricsReader = (*redisState)(nil)
...
func (s *redisState) CommitLeasedNode(ctx context.Context, req engine.CommitNodeRequest) (engine.CommitNodeResult, error) {
...
func (s *redisState) ResetNodeForRetryWithOutbox(ctx context.Context, id types.ExecutionID, nodeName string, token engine.LeaseToken, entry engine.OutboxEntry) (bool, error) {
	if entry.ID == "" {
...
func (s *redisState) RevokeLeaseWithOutbox(ctx context.Context, id types.ExecutionID, nodeName string, token engine.LeaseToken, entry engine.OutboxEntry) (bool, error) {
	if token == "" {
		return false, nil
	}
	if entry.ID == "" {
...
func (s *redisState) CommitNode(ctx context.Context, req engine.CommitNodeRequest) (engine.CommitNodeResult, error) {
...
func (s *redisState) AdvanceNode(ctx context.Context, req engine.AdvanceNodeRequest) (engine.AdvanceNodeResult, error) {
...
func (s *redisState) ListOutbox(ctx context.Context, id types.ExecutionID, before time.Time, limit int) ([]engine.OutboxEntry, error) {
	if limit <= 0 {
		return nil, nil
	}
...
func (s *redisState) AckOutbox(ctx context.Context, id types.ExecutionID, entryID string) error {
...
func (s *redisState) RecordOutboxFailure(ctx context.Context, id types.ExecutionID, entryID string, maxAttempts int) (engine.OutboxDeliveryFailure, error) {
	if maxAttempts <= 0 {
...
func (s *redisState) OutboxMetrics(ctx context.Context) (engine.OutboxMetricsSnapshot, error) {
...
func (s *redisState) ListOutboxExecutions(ctx context.Context, limit int) ([]types.ExecutionID, error) {
	if limit <= 0 {
		return nil, nil
	}
...
func (s *redisState) evictExecutionCaches(id types.ExecutionID) {
	s.mu.Lock()
	delete(s.graphs, id)
	s.mu.Unlock()
	s.ttlMu.Lock()
