...
func cleanupWorkflowRegistryRecord(t *testing.T, rdb *redis.Client, rec backend.WorkflowRecord) {
	...
	if err := rdb.Del(context.Background(), workflowByIDKey(rec.ID), workflowByKeyKey(rec.Key)).Err(); err != nil {
		t.Fatalf("Del(%q, %q) error = %v", workflowByIDKey(rec.ID), workflowByKeyKey(rec.Key), err)
	}
}
