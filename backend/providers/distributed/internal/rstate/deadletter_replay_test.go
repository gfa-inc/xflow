...
func seedDeadLetter(t *testing.T, state *Store, id types.ExecutionID, entryID string) engine.OutboxEntry {
	...
	if err := state.rdb.Set(ctx, nodeStatusKey(namespace.Default, id, "review"), "running", time.Minute).Err(); err != nil {
		t.Fatalf("set node status: %v", err)
	}
	if err := state.rdb.HSet(ctx, nodeMetaKey(namespace.Default, id, "review"), "activation_id", 1).Err(); err != nil {
...
func TestReplayDeadLetterRejectsTerminalExecution(t *testing.T) {
	...
	if err := state.rdb.Set(ctx, execKey(namespace.Default, id, "status"), "success", time.Minute).Err(); err != nil {
...
func TestReplayDeadLetterRejectsInactiveExecution(t *testing.T) {
	...
	if err := state.rdb.Del(ctx, execKey(namespace.Default, id, "status")).Err(); err != nil {
...
func TestReplayDeadLetterRejectsNodeTerminal(t *testing.T) {
	...
	if err := state.rdb.Set(ctx, nodeStatusKey(namespace.Default, id, "review"), "success", time.Minute).Err(); err != nil {
...
func TestReplayDeadLetterRejectsActivationMismatch(t *testing.T) {
	...
	if err := state.rdb.HSet(ctx, nodeMetaKey(namespace.Default, id, "review"), "activation_id", 2).Err(); err != nil {
...
func TestReplayDeadLetterBodyPreserved(t *testing.T) {
	...
	deadBody, err := state.rdb.HGet(ctx, outboxDeadBodyKey(namespace.Default, id), entryID).Result()
	if err != nil {
	...
	readyBody, err := state.rdb.HGet(ctx, outboxBodyKey(namespace.Default, id), entryID).Result()
	if err != nil {
...
func TestListDeadLettersCursorPagination(t *testing.T) {
	...
	for i := 0; i < total; i++ {
		...
		if err := state.rdb.HSet(ctx, outboxDeadBodyKey(namespace.Default, id), entryID, body).Err(); err != nil {
			t.Fatalf("HSet: %v", err)
		}
		if err := state.rdb.ZAdd(ctx, outboxDeadKey(namespace.Default, id), redis.Z{Score: float64(i), Member: entryID}).Err(); err != nil {
			t.Fatalf("ZAdd: %v", err)
		}
		if err := state.rdb.HSet(ctx, outboxDeadMetaKey(namespace.Default, id, entryID), "node", "review", "activation", "1", "intent", "execute", "task_type", "0").Err(); err != nil {
...
func seedDeadLetterNoMeta(t *testing.T, state *Store, id types.ExecutionID, entryID string) {
	...
	if err := state.rdb.ZAdd(ctx, outboxDeadKey(namespace.Default, id), redis.Z{Score: float64(time.Now().UTC().UnixMilli()), Member: entryID}).Err(); err != nil {
...
func seedDeadLettersBulk(t *testing.T, state *Store, id types.ExecutionID, scores []float64, members []string) {
	...
	for i, m := range members {
		...
		if err := state.rdb.HSet(ctx, outboxDeadBodyKey(namespace.Default, id), m, body).Err(); err != nil {
			t.Fatalf("HSet body: %v", err)
		}
		if err := state.rdb.ZAdd(ctx, outboxDeadKey(namespace.Default, id), redis.Z{Score: scores[i], Member: m}).Err(); err != nil {
			t.Fatalf("ZAdd: %v", err)
		}
		if err := state.rdb.HSet(ctx, outboxDeadMetaKey(namespace.Default, id, m), "node", "review", "activation", "1", "intent", "execute", "task_type", "0").Err(); err != nil {
			t.Fatalf("HSet meta: %v", err)
		}
	}
}
...
func TestListDeadLettersCursorSameScoreBeyondInternalPageNoDrop(t *testing.T) {
	state, _, _ := newTestRedisState(t)
	ctx := context.Background()
	id := types.ExecutionID("dl-list-samescore-large")
	const total = 650
	members := make([]string, total)
	scores := make([]float64, total)
	for i := 0; i < total; i++ {
		members[i] = fmt.Sprintf("execute/dl-list-samescore-large/review/m%03d", i)
		scores[i] = 100
	}
	seedDeadLettersBulk(t, state, id, scores, members)
	...
	cursor := ""
	for pages := 0; ; pages++ {
		list, err := state.ListDeadLetters(ctx, id, engine.DeadLetterPage{Limit: 100, Cursor: cursor})
		if err != nil {
			t.Fatalf("ListDeadLetters cursor=%q: %v", cursor, err)
		}
		for _, e := range list.Entries {
...
func TestListDeadLettersConcurrentInsertNoDrop(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < 8; i++ {
			...
			_ = state.rdb.HSet(ctx, outboxDeadBodyKey(namespace.Default, id), m, body).Err()
			_ = state.rdb.ZAdd(ctx, outboxDeadKey(namespace.Default, id), redis.Z{Score: float64(100 + i), Member: m}).Err()
			_ = state.rdb.HSet(ctx, outboxDeadMetaKey(namespace.Default, id, m), "node", "review", "activation", "1").Err()
...
func TestListDeadLettersDeleteBoundaryEntryNoDrop(t *testing.T) {
	...
	listDeadLettersAfterFetchHook = func() {
		once.Do(func() {
			_ = state.rdb.ZRem(ctx, outboxDeadKey(namespace.Default, id), "d2").Err()
		})
	}
...
func TestListDeadLettersDeleteDuringPagination(t *testing.T) {
	...
	if err := state.rdb.HDel(ctx, outboxDeadBodyKey(namespace.Default, id), "d3").Err(); err != nil {
		t.Fatalf("HDel: %v", err)
	}
	...
	remaining, err := state.rdb.ZCard(ctx, outboxDeadKey(namespace.Default, id)).Result()
	if err != nil {
...
func TestReplayDeadLetterFailClosedOnMissingNodeGuardState(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			wantExact:   true,
			prepare: func(t *testing.T, state *Store, id types.ExecutionID) {
				if err := state.rdb.Del(ctx, nodeStatusKey(namespace.Default, id, "review")).Err(); err != nil {
		...
		},
		{
			...
			wantExact:   true,
			prepare: func(t *testing.T, state *Store, id types.ExecutionID) {
				if err := state.rdb.Del(ctx, nodeMetaKey(namespace.Default, id, "review")).Err(); err != nil {
		...
		},
		{
			...
			wantExact:   true,
			prepare: func(t *testing.T, state *Store, id types.ExecutionID) {
				if err := state.rdb.HDel(ctx, nodeMetaKey(namespace.Default, id, "review"), "activation_id").Err(); err != nil {
		...
		},
		{
			...
			wantExact:   true,
			prepare: func(t *testing.T, state *Store, id types.ExecutionID) {
				if err := state.rdb.Set(ctx, nodeStatusKey(namespace.Default, id, "review"), "bogus", time.Minute).Err(); err != nil {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if first.Outcome == engine.ReplayReplayed {
				...
				attempts, err := state.rdb.HExists(ctx, outboxAttemptsKey(namespace.Default, id), entryID).Result()
				if err != nil {
				...
				receiptHash, err := state.rdb.HGetAll(ctx, outboxReplayReceiptKey(namespace.Default, id, "req-guard")).Result()
				if err != nil {
...
func TestReplayDeadLetterFailClosedConcurrentNoLeak(t *testing.T) {
	...
	dead, err := state.rdb.ZCard(ctx, outboxDeadKey(namespace.Default, id)).Result()
	if err != nil {
	...
	ready, err := state.rdb.ZCard(ctx, outboxReadyKey(namespace.Default, id)).Result()
	if err != nil {
...
func TestAdvanceNodeActivationFenceRealLifecycle(t *testing.T) {
	...
	if err := state.rdb.HSet(ctx, nodeMetaKey(namespace.Default, id, "start"), "activation_id", 1).Err(); err != nil {
		t.Fatalf("bump source activation: %v", err)
	}
	if err := state.rdb.Set(ctx, nodeStatusKey(namespace.Default, id, "start"), "success", time.Minute).Err(); err != nil {
	...
	inDegree, err := state.rdb.Get(ctx, inDegreeKey(namespace.Default, id, 1)).Int()
	...
	active, err := state.rdb.Get(ctx, activeInputsKey(namespace.Default, id, 1)).Int()
	if err != nil && err != redis.Nil {
	...
	if err := state.rdb.HSet(ctx, nodeMetaKey(namespace.Default, id, "start"), "activation_id", 0).Err(); err != nil {
...
func TestAdvanceNodeFailClosedOnMissingSourceActivation(t *testing.T) {
	...
	inDegree := inDegreeKey(namespace.Default, id, 1)
	...
	marker := advanceMarkerKey(namespace.Default, id, "start", 0)
