...
func TestUpsertNodeIndexesLeaseAndUnsindexesOnTerminal(t *testing.T) {
	...
	got, err := rdb.ZScore(ctx, leaseExpiryZSetKey(namespace.Default, "e1"), leaseExpiryMember("e1", "n")).Result()
	if err != nil {
	...
	if _, err := rdb.ZScore(ctx, leaseExpiryZSetKey(namespace.Default, "e1"), leaseExpiryMember("e1", "n")).Result(); err != redis.Nil {
...
func TestRevokeLeaseAtomicallyRollsBackWhenTokenMatches(t *testing.T) {
	...
	status, err := rdb.Get(ctx, nodeStatusKey(namespace.Default, "e1", "n")).Result()
	if err != nil {
		t.Fatal(err)
	}
	...
	tok, _ := rdb.HGet(ctx, nodeMetaKey(namespace.Default, "e1", "n"), "lease_token").Result()
	if tok != "" {
...
func TestRevokeLeaseLosesRaceToConcurrentCommit(t *testing.T) {
	...
	if err := rdb.Set(ctx, nodeStatusKey(namespace.Default, "e1", "n"), string(types.NodeStatusCommitting), time.Minute).Err(); err != nil {
		t.Fatal(err)
	}
	if err := rdb.HSet(ctx, nodeMetaKey(namespace.Default, "e1", "n"), "lease_token", "").Err(); err != nil {
		t.Fatal(err)
	}
...
func TestListExpiredLeasesPrunesStaleIndexEntries(t *testing.T) {
	...
	if err := rdb.ZAdd(ctx, leaseExpiryZSetKey(namespace.Default, "e1"), redis.Z{
	...
	if _, err := rdb.ZScore(ctx, leaseExpiryZSetKey(namespace.Default, "e1"), leaseExpiryMember("e1", "gone")).Result(); err != redis.Nil {
