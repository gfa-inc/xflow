...
// every authoritative Redis replay receipt across namespaces and decodes the
...
func TestScanReplayReceiptsProjectsAllReceipts(t *testing.T) {
	...
	// Seed two receipts in namespace-a and one in namespace-b.
	seedReceipt(t, state, "namespace-a", "exec-a-1", "req-a-1", "audit-a-1", "review", "1", "replayed")
	seedReceipt(t, state, "namespace-a", "exec-a-2", "req-a-2", "audit-a-2", "approve", "2", "rejected_terminal")
	seedReceipt(t, state, "namespace-b", "exec-b-1", "req-b-1", "audit-b-1", "review", "1", "replayed")
	...
	if got[0].AuditID != "audit-a-1" || got[0].Namespace != "namespace-a" || got[0].ExecutionID != "exec-a-1" {
...
func TestScanReplayReceiptsSkipsEmptyAuditID(t *testing.T) {
	...
	seedReceipt(t, state, "namespace-a", "exec-good", "req-good", "audit-good", "n", "1", "replayed")
	seedMalformedReceipt(t, state, "namespace-a", "exec-bad", "req-bad")
...
func TestScanReplayReceiptsAbortOnError(t *testing.T) {
	...
	seedReceipt(t, state, "namespace-a", "exec-abort", "req-abort", "audit-abort", "n", "1", "replayed")
	boom := errScanAbort{}
	err := state.ScanReplayReceipts(ctx, func(r engine.ReplayReceipt) error {
		return boom
	})
...
func seedReceipt(t *testing.T, state *Store, namespaceName, execID, requestID, auditID, node, activation, outcome string) {
	t.Helper()
	ctx := context.Background()
	// Register the namespace so listNamespaces (the scan fan-out source) includes it.
	if err := state.registerNamespace(ctx, namespace.Namespace(namespaceName)); err != nil {
		t.Fatalf("register namespace: %v", err)
	}
	key := outboxReplayReceiptKey(namespace.Namespace(namespaceName), types.ExecutionID(execID), requestID)
...
func seedMalformedReceipt(t *testing.T, state *Store, namespaceName, execID, requestID string) {
	t.Helper()
	ctx := context.Background()
	if err := state.registerNamespace(ctx, namespace.Namespace(namespaceName)); err != nil {
		t.Fatalf("register namespace: %v", err)
...
func TestParseReplayReceiptKey(t *testing.T) {
	cases := []struct {
		...
		{"xflow:ns:namespace-a:exec:{exec-1}:replay:receipt:req-1", "exec-1", "req-1", true},
		{"xflow:ns:namespace-a:exec:{exec-1}:replay:receipt:req:with:colons", "exec-1", "req:with:colons", true},
		{"xflow:ns:namespace-a:exec:{exec-1}:outbox:dead", "", "", false},
		{"xflow:ns:namespace-a:exec:{exec-1}:replay:receipt:", "", "", false},
