...
// Key schema (Redis Cluster-safe, G2 Phase 2 / Task 2.1 + Task 7.2 namespace scope).
//
...
//
//	xflow:ns:<namespace>:workflow:{<key>}:bykey        -> workflow ID (the "exists" pointer)
//	xflow:ns:<namespace>:workflow:{<key>}:byid:<id>    -> storedWorkflowRecord payload (JSON)
//
...
// same slot regardless of <id>. The namespace prefix is intentionally brace-less:
// had it been `{namespace}`, the first {...} would steal the hash tag and collapse
// an entire namespace's workflows onto one slot. With `ns:<namespace>` the first `{`
...
// `{<key>}`-tagged key. We therefore keep a per-namespace reverse index
//
//	xflow:ns:<namespace>:workflow:idmap:<id>           -> key   (no hash tag, single-key op)
//
...
func workflowByIDKey(t namespace.Namespace, key string, id types.WorkflowID) string {
	return "xflow:ns:" + string(t) + ":workflow:{" + key + "}:byid:" + string(id)
}

// workflowByIDKeyPrefix returns the byid key prefix INCLUDING the `{<key>}`
// hash tag and the brace-less namespace prefix. The addWorkflowRecordLua script
...
func workflowByIDKeyPrefix(t namespace.Namespace, key string) string {
	return "xflow:ns:" + string(t) + ":workflow:{" + key + "}:byid:"
}
...
func workflowDefHashKey(t namespace.Namespace, key string, id types.WorkflowID) string {
...
func workflowDefHashKeyPrefix(t namespace.Namespace, key string) string {
	return "xflow:ns:" + string(t) + ":workflow:{" + key + "}:defhash:"
}
...
// because the byid record lives in the `{<key>}` hash-tagged slot. The namespace
// must come from the request context (namespace.FromContext), never a client
// request body.
func KeyByID(t namespace.Namespace, key string, id types.WorkflowID) string {
	return workflowByIDKey(t, key, id)
}
func KeyByKey(t namespace.Namespace, key string) string          { return workflowByKeyKey(t, key) }
func KeyIDMap(t namespace.Namespace, id types.WorkflowID) string { return workflowIDMapKey(t, id) }
